...
#include "../sprites/bac_clock.h"
#include "../sprites/bac_giant.h"
#include "../sprites/bac_house_alien.h"
#include "../sprites/bac_wind.h"
#include "../sprites/bac_castle.h"
#include "../sprites/bac_skyscraper_alien.h"
#include "../sprites/bac_ufo2.h"
#include "../sprites/bac_moon2.h"
#include "../sprites/bac_planet2.h"
#include "../sprites/bac_city2.h"
...
void stage_game2_init(void)
{
    ...
    bac_elem1_tlut = tlut_copy(bac_clock_tlut_day, 3);
    bac_elem2_tlut = tlut_copy(bac_mountains_tlut_day, 7);
    bac_elem3_tlut = tlut_copy(bac_giant_tlut_day, 3);
    bac_elem4_tlut = tlut_copy(bac_house_alien_tlut_day, 3);
    bac_elem5_tlut = tlut_copy(bac_wind_tlut_sunset, 4);
    bac_elem6_tlut = tlut_copy(bac_castle_tlut_sunset, 3);
    bac_elem7_tlut = tlut_copy(bac_skyscraper_alien_tlut_sunset, 3);
    bac_elem8_tlut = tlut_copy(bac_ufo2_tlut_sunset, 3);
    bac_elem9_tlut = tlut_copy(bac_moon2_tlut_night, 2);
    bac_elem10_tlut = tlut_copy(bac_planet2_tlut_night, 3);
    bac_elem11_tlut = tlut_copy(bac_city2_tlut_night, 4);
    ...
    */
    ...
    {
        int index = instance_create(OBJ_PYORO2, 160+16, 189); // Player 2
        char value = 1;
        instance_create(OBJ_PYORO2, 160-16, 189); // Player 1
        if (index != -1)
            instance_set(index, SET_PLAYER, &value);
    }
    else
        instance_create(OBJ_PYORO2, 160, 189);
...
void stage_game2_update(void)
{
    ...
    // Make sure there aren't too many bean leaves
    while ((instance_count(OBJ_LEAF) + instance_count(OBJ_LEAF_SMALL)) > MAX_LEAVES)
    {
        int i;
        OSTime lasttime = osGetTime();
        int lastindex = -1;
        
        // Search through all objects that are big or small leaves
        for (i=0;i<MAXOBJECTS;i++)
        {
            if (GameObjects[i].type == OBJ_LEAF || GameObjects[i].type == OBJ_LEAF_SMALL)
            {
                // Get its time of creation
                OSTime objtime = *(OSTime*)instance_get(i, GET_TIME);
                
                // If the creation time is smaller than the last time we measured, mark that object for removal
                if (OS_CYCLES_TO_USEC(objtime) < OS_CYCLES_TO_USEC(lasttime))
                {
                    lasttime = objtime;
                    lastindex = i;
                }
        ...
        // Destroy the index (the if is not needed but, just in case...)
        if (lastindex != -1)
            instance_destroy_index(lastindex);
    }
    
    // Play a jingle if Pyoro is dead
    if (instance_find(OBJ_PYORO2) == -1 && game_over == 0)
    {
        u32 hscore = (global_save_highscores[7] << 24) | (global_save_highscores[6] << 16) | (global_save_highscores[5] << 8) | global_save_highscores[4];
        game_over = 1;
        ...
        // Save score if in singleplayer
        if (global_save_settings[SAVE_SETTINGS_PLAYER] == 0)
        {
            // Store the highscore
            if (GameScore > hscore)
            {
                global_save_highscores[7] = (GameScore >> 24) & 0xFF;
                global_save_highscores[6] = (GameScore >> 16) & 0xFF;
                global_save_highscores[5] = (GameScore >> 8) & 0xFF;
                global_save_highscores[4] = GameScore & 0xFF;
    ...
    if ((bean3_spawnangels == 0 && bean3_nextangel == 0 && bean3_destruction_y != 0) || (bean3_nextangel < osGetTime() && bean3_nextangel != 0))
    {
        ...
        for (i=0;i<=28;i++)
        {
            ...
            {
                int index = instance_find_withvalue(OBJ_PYORO2, GET_PLAYER, &bean3_destruction_player);
                float pyoro_pos;
                int xpos = 49+8*i;
    ...
    else if (GameScore >= DARK_MODE)
    {
        ...
        tlut_transition(bac_elem4_tlut, bac_house_alien_tlut_dark, 3);
        tlut_transition(bac_elem5_tlut, bac_wind_tlut_dark, 4);
        tlut_transition(bac_elem6_tlut, bac_castle_tlut_dark, 3);
        tlut_transition(bac_elem7_tlut, bac_skyscraper_alien_tlut_dark, 3);
        tlut_transition(bac_elem8_tlut, bac_ufo2_tlut_dark, 3);
        tlut_transition(bac_elem10_tlut, bac_planet2_tlut_dark, 3);
        tlut_transition(bac_elem11_tlut, bac_city2_tlut_dark, 4);
    ...
    else if (GameScore >= BEIGE_MODE)
    {
        ...
        tlut_transition(bac_elem4_tlut, bac_house_alien_tlut_beige, 3);
        tlut_transition(bac_elem5_tlut, bac_wind_tlut_beige, 4);
        tlut_transition(bac_elem6_tlut, bac_castle_tlut_beige, 3);
        tlut_transition(bac_elem7_tlut, bac_skyscraper_alien_tlut_beige, 3);
        tlut_transition(bac_elem8_tlut, bac_ufo2_tlut_beige, 3);
        tlut_transition(bac_elem9_tlut, bac_moon2_tlut_beige, 2);
        tlut_transition(bac_elem10_tlut, bac_planet2_tlut_beige, 3);
        tlut_transition(bac_elem11_tlut, bac_city2_tlut_beige, 4);
    ...
    else if (GameScore >= NIGHT_MODE)
    {
        ...
        tlut_transition(bac_elem2_tlut, bac_mountains_tlut_night, 7);
        tlut_transition(bac_elem1_tlut, bac_clock_tlut_night, 3);
        tlut_transition(bac_elem3_tlut, bac_giant_tlut_night, 3);
        tlut_transition(bac_elem4_tlut, bac_house_alien_tlut_night, 3);
        tlut_transition(bac_elem5_tlut, bac_wind_tlut_night, 4);
        tlut_transition(bac_elem6_tlut, bac_castle_tlut_night, 3);
        tlut_transition(bac_elem7_tlut, bac_skyscraper_alien_tlut_night, 3);
        tlut_transition(bac_elem8_tlut, bac_ufo2_tlut_night, 3);
    ...
    else if (GameScore >= SUNSET_MODE)
    {
        ...
        if (GameSpeed < 50)
            GameSpeed = 50;
        tlut_transition(bac_elem2_tlut, bac_mountains_tlut_sunset, 7);
        tlut_transition(bac_elem4_tlut, bac_house_alien_tlut_sunset, 3);
        col1.r = col1.r + (((float)(col1_r_to - col1.r))/((float)30));
        col1.g = col1.g + (((float)(col1_g_to - col1.g))/((float)30));
        col1.b = col1.b + (((float)(col1_b_to - col1.b))/((float)30));
        if (extra == 0)
    ...
    if (nextspeed_timer < osGetTime() && GameSpeed < 100 && instance_find(OBJ_PYORO2) != -1)
    {
        GameSpeed++;
        if (GameSpeed > 90)
...
void DrawLandscape()
{
    ...
    if (bac_elem12_alpha > 0)
    {
        ...
        gDPLoadTLUT_pal16(glistp++, 0, bac_comet_tlut_red);
        for(i = 0; i < h; i++)
        {
    ...
    gDPLoadMultiTile_4b(glistp++, bac_wind, 0, G_TX_RENDERTILE, G_IM_FMT_CI, w, h, 0, 0, w - 1, h - 1, 0, G_TX_WRAP, G_TX_WRAP, 0, 0, G_TX_NOLOD, G_TX_NOLOD);
    ...
    for(i = 0; i < h; i++)
    {
        gDPLoadMultiTile_4b(glistp++, bac_castle, 0, G_TX_RENDERTILE, G_IM_FMT_CI, w, h, 0, i, w - 1, i, 0, G_TX_WRAP, G_TX_WRAP, 0, 0, G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(glistp++, x << 2, y + i << 2, x + w << 2, y + i+1 << 2,  G_TX_RENDERTILE,  0 << 5, i << 5,  1 << 10, 1 << 10);
    }
    
    // Draw the alien skyscraper
    x = 100;
    y = 83;
    w = 12;
    h = 78;
    ...
    for(i = 0; i < h; i++)
    {
        gDPLoadMultiTile_4b(glistp++, bac_skyscraper_alien, 0, G_TX_RENDERTILE, G_IM_FMT_CI, w, h, 0, i, w - 1, i, 0, G_TX_WRAP, G_TX_WRAP, 0, 0, G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(glistp++, x << 2, y + i << 2, x + w << 2, y + i+1 << 2,  G_TX_RENDERTILE,  0 << 5, i << 5,  1 << 10, 1 << 10);
    }
    ...
    gDPLoadTextureBlock_4b(glistp++, bac_ufo2, G_IM_FMT_CI, w, h, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadTextureBlock_4b(glistp++, bac_moon2, G_IM_FMT_CI, w, h, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadTextureBlock_4b(glistp++, bac_planet2, G_IM_FMT_CI, w, h, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadTextureBlock_4b(glistp++, bac_city2, G_IM_FMT_CI, w, h, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadMultiTile_4b(glistp++, bac_clock, 0, G_TX_RENDERTILE, G_IM_FMT_CI, w, h, 0, 0, w - 1, h - 1, 0, G_TX_WRAP, G_TX_WRAP, 0, 0, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadMultiTile_4b(glistp++, bac_house_alien, 0, G_TX_RENDERTILE, G_IM_FMT_CI, w, h, 0, 0, w - 1, h - 1, 0, G_TX_WRAP, G_TX_WRAP, 0, 0, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
    ...
    gDPLoadTextureBlock_4b(glistp++, bac_giant, G_IM_FMT_CI, w, h, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gSPTextureRectangle(glistp++, x<<2, y<<2, x+w<<2, y+h<<2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10 );
...
void DrawHUD()
{
    ...
    char conbuff[7];
    char conbuff2[7];
    u32 highscore = (global_save_highscores[7] << 24) | (global_save_highscores[6] << 16) | (global_save_highscores[5] << 8) | global_save_highscores[4];
    ...
    // Get some values for display if debug mode is on
        ...
        sprintf(conbuff6, "%03d", instance_count(OBJ_LEAF) + instance_count(OBJ_LEAF_SMALL));
    #endif
    ...
    // Show debug information
        ...
        for (i=0;i<sizeof(conbuff6)-1;i++)
        {
            char offset = conbuff6[i] - '0';
            char pos = i*8;
            gDPLoadTextureTile_4b(glistp++, fnt_numbers, G_IM_FMT_CI, 8, 8, 0, offset*8, 8, 8 + (offset*8), 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
            gSPTextureRectangle(glistp++, 16+pos << 2, 64 << 2, 16+8+pos << 2, 64+8 << 2,  G_TX_RENDERTILE,  0 << 5, offset*8 << 5,  1 << 10, 1 << 10);
        }
    #endif
    gDPPipeSync(glistp++);
