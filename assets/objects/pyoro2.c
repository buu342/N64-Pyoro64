...
void create_pyoro2(BASE *obj_base, float x, float y)
{
    ...
    self->player = 0;
    self->frozen = 0;
    self->tlut = tlut_copy(spr_pyoro_tlut_yellow, 5);
...
void update_pyoro2(BASE *obj_base)
{
    ...
    // If we are in the attack state, do the attack animation. Otherwise go to idle
    if (self->state == STATE_ATTACK)
    {
        ...
        {
            self->frame_num++;
            self->frame_timer = osGetTime() + OS_USEC_TO_CYCLES_CORRECT(10000);
            switch (self->frame_num)
            {
                case 1:
                    self->sprite = spr_pyoro_spit1;
                    break;
                case 2:
                    self->sprite = spr_pyoro_spit2;
                    break;
                case 3:
                    self->sprite = spr_pyoro_spit3;
                    break;
                case 4:
                    self->sprite = spr_pyoro_spit4;
                    break;
                case 5:
                    self->sprite = spr_pyoro_walk1;
                    break;
    ...
    else if (stick1_x > 16 || stick1_x < -16)
    {
        // Set the state
    ...
    if (col_index != -1)
    {
        ...
        if ((DEBUG_MODE && !(contdata[self->player].button & Z_TRIG)) || !DEBUG_MODE)
        {
            ...
            int otherplayer_index = instance_find_withvalue(OBJ_PYORO2, GET_PLAYER, &otherplayer);
    ...
    // Allow the player to shoot seeds
    if (PlayerIsAttacking(self->player, 0) && self->state != STATE_ATTACK && self->state != STATE_DIE)
    {
        int index = instance_create(OBJ_SEEDS, self->x, self->y);
        if (index != -1)
        {
            self->state = STATE_ATTACK;
            self->sprite = spr_pyoro_spit1;
            self->frame_num = 0;
            self->frame_timer = osGetTime();
            instance_set(index, SET_PLAYER, &(self->player));
            instance_set(index, SET_DIR, &(self->direction));
            *sndHandle_player = nuAuStlSndPlayerPlay(SND_SPIT);
        }
    }
}
...
void set_pyoro2(BASE *obj_base, u8 what, void* value)
{
    switch(what)
    {
        ...
        case SET_PLAYER:
            ...
            {
                free(self->tlut);
                self->tlut = tlut_copy(spr_pyoro_tlut_blue, 5);
            }
            break;
        case SET_FROZEN:
...
void draw_pyoro2(BASE *obj_base)
{
    ...
    if (self->player == 1)
    {
        accessory_index = spr_pyoro_hair3;
        if (self->sprite == spr_pyoro_walk1 || self->sprite == spr_pyoro_chew2 || self->sprite == spr_pyoro_spit4)
        {
            accessory_x = 8;
            accessory_y = -4;
        }
        ...
        else if (self->sprite == spr_pyoro_spit1)
        {
            accessory_x = 5;
            accessory_y = -6;
        }
        else if (self->sprite == spr_pyoro_spit2)
        {
            accessory_x = 5;
            accessory_y = -6;
        }
        else if (self->sprite == spr_pyoro_spit3)
        {
            accessory_x = 6;
            accessory_y = -5;
        }
    ...
    // Choose what sprite the mouth has depending on Pyoro's sprite
    if (self->state == STATE_ATTACK)
    {
        switch (self->frame_num)
        {
            case 1:
                mouth_index = spr_mouth_spit1;
                break;
            case 2:
                mouth_index = spr_mouth_spit2;
                break;
            case 3:
                mouth_index = spr_mouth_spit3;
                break;
        }
    }
    ...
    if (self->direction == DIR_LEFT)
    {
        ...
        if (mouth_index != NULL)
        {
            gDPLoadTextureTile_4b(glistp++, mouth_index, G_IM_FMT_CI, 8, 16, 0, 0, 8-1, 16-1, 0, G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
            gSPTextureRectangle(glistp++, (int)self->x-4-11 << 2, (int)self->y-8-8 << 2, (int)self->x+4-11 << 2, (int)self->y+8-8 << 2, G_TX_RENDERTILE, 0 << 5, 0 << 5, 1 << 10, 1 << 10);
        }
    }
    else // Flip the texture
    {
        ...
        if (mouth_index != NULL)
        {
            gDPLoadTextureTile_4b(glistp++, mouth_index, G_IM_FMT_CI, 8, 16, 0, 0, 8-1, 16-1, 0, G_TX_MIRROR, G_TX_WRAP, 3, 4, G_TX_NOLOD, G_TX_NOLOD);
            gSPTextureRectangle(glistp++, (int)self->x-4+12 << 2, (int)self->y-8-8 << 2, (int)self->x+4+12 << 2, (int)self->y+8-8 << 2, G_TX_RENDERTILE, 8 << 5, 0 << 5, 1 << 10, 1 << 10);
        }
    }
