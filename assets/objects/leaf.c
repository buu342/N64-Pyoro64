...
void update_leaf(BASE *obj_base)
{
    ...
    if ((self->y > 220 || self->x < 42 || self->x > 282) && index != -1)
    {
    ...
    // if out of HP, destroy self but create two smaller leaves
    if (self->health <= 0 && index != -1)
    {
        char i;
        for (i=0;i<2;i++)
        {
            int leaf_index = instance_create(OBJ_LEAF_SMALL, self->x+(-4+guRandom()%8), self->y-(2-guRandom()%5));
            if (leaf_index != -1)
            {
                instance_set(leaf_index, SET_TYPE, &self->type);
                instance_set(leaf_index, SET_SPEED, &self->hspeed);
            }
        }
    ...
    if (self->frame_timer < osGetTime())
    {
        ...
        switch (self->frame)
        {
            ...
            case 2:
            case 5:
                self->sprite = spr_bean_leaf_big2;
                break;
            case 3:
            case 4:
                self->sprite = spr_bean_leaf_big3;
                break;
        }
    }
...
void set_leaf(BASE *obj_base, u8 what, void* value)
{
    switch(what)
    {
        case SET_SPEED:
            ...
            // Temporarily disable collision checking to avoid while loop always checking the same object
